/*
 ============================================================================
 Project    : Opposition-Based Memetic Algorithm for the SUPBR Problem
 File       : obma.c
...
static int start_obma(parameters *params, int *num_eval_fit_function, int *num_used_generations);
...
int main(int argc, char **argv){
    ...
    score = start_obma(&params, &num_eval_fit_function, &num_used_generations);
...
static int start_obma(parameters *params, int *num_eval_fit_function, int *num_used_generations)
{
    ...
    if (start_breeding == TRUE){
        ...
        for(generation_number=2; generation_number<=number_of_generations; generation_number++){
            ...
            replacement(&pop); 
            LS_for_population(&pop,generation_number,num_eval_fit_function); 
            if (last_best==best_solution)
                count++;
            else
                count = 0;
            last_best=best_solution;
            if (obma_reached_degenerate_state(&pop)||count==14){
                restart_population_OBL_LS(&pop, &perm,num_eval_fit_function);
                count=0;
            }
...
static int start_obma(parameters *params, int *num_eval_fit_function, int *num_used_generations)
{
    ...
    generate_initial_population(&pop,&perm);
    calculate_fitness_population(&pop,1,num_eval_fit_function);
    OBL_LS_for_population(&pop,1,num_eval_fit_function,&flag_obl);
    best_solution = pop.chromosomes[0].fitness;
    number_of_generations = perm.length;
    ...
    if (start_breeding == TRUE){
        count=0;
        last_best=-1;
        flag_obl=2; //maximum number to skip applicatios of obl
        for(generation_number=2; generation_number<=number_of_generations; generation_number++){
            selection(&pop,generation_number,&best_solution);
            crossover(&pop, &perm);
            mutation(&pop, &perm);
            ...
            last_best=best_solution;
            if (obma_reached_degenerate_state(&pop)||count==14){
                restart_population_OBL_LS(&pop, &perm,num_eval_fit_function,&flag_obl);
                count=0;
            }
...
/*
int start_obma(int *num_eval_fit_function, int pnumber_generations){
    //Variables
    int i,generation_number;
    permutation perm;
    population pop;
    ...
    generate_initial_population(&pop,&perm);
    calculate_fitness_population(&pop,1,num_eval_fit_function);
    //LS_for_population(&pop,1,num_eval_fit_function); //A
    OBL_LS_for_population(&pop,1,num_eval_fit_function,&flag_obl); //B
    best_solution = pop.chromosomes[0].fitness;
    number_of_generations = perm.length;
    if (pnumber_generations > -1) number_of_generations = pnumber_generations;
    ...
    for(generation_number=2; generation_number<=number_of_generations; generation_number++){
        ...
        //OBL_LS_for_population(&pop,generation_number,num_eval_fit_function,&flag_obl); //A
        LS_for_population(&pop,generation_number,num_eval_fit_function); //B
        ...
        //if (obma_reached_degenerate_state(&pop)||count==15){// A
        if (obma_reached_degenerate_state(&pop)||count==14){// B
            restart_population_OBL_LS(&pop, &perm,num_eval_fit_function,&flag_obl);
            count=0;
        }
        
        //printf("\tGeneration %d,  Fitness %d \n",generation_number,best_solution);
        printf("%d ",best_solution); //print best solution by generation
    }
