 ...
 */
void restart_population_OBL_LS(population *pob, permutation *perm, int *num_eval_fit_function){
    int i,j, prob, conservation_size;
 ...
 */
void OBL_LS_for_population(population *pob, int *num_eval_fit_function){
    int i; int count_obl=0, count_ls=0;
    
    //generation_number == 1, is the initial population
    for(i=pob->population_base; i < pob->population_top; i++){
        if (opposite_permutation(pob, i, 0, pob->chromosomes[0].length,num_eval_fit_function)){
            count_obl++;
        }
